...
class CPG:
    def __init__(self):
        ...
        self.rate = rospy.Rate(2*self.pi/0.04)
    ...
    def update_values(self,amp_target = 1.56, offset_target = np.array([0,0]), frequency_target = [0.04,0.04], phase_offset_target = 0):
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            self.update_values()
            self.output()
            self.tf_res[0] = self.print1(self.result[0])
            j=[self.tf_res[0],self.l1j2,self.l1j3,self.tf_res[0],self.l2j2,self.l2j3,self.tf_res[0],self.l3j2,self.l3j3]

            # self.msg1.data = self.print1(self.result[0])
            # self.msg2.data = self.print1(self.result[1])

            self.msg1.data = j
            self.msg2.data = self.j2
